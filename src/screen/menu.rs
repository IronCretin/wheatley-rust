...
use tcod::colors;
use tcod::console::{BackgroundFlag, Console, Root, TextAlignment};
...
impl Screen for MenuScreen {
    fn render(&self, _game: &mut Game, display: &mut Root) {
        ...
        for line in self.title.lines() {
            display.print_ex(x, y, BackgroundFlag::None, TextAlignment::Center, line);
            y += 1;
        }
        y += 1;
        for (i, (opt, _)) in self.options.iter().enumerate() {
            if i == self.active.get() {
                ...
                display.print_ex(x, y, BackgroundFlag::None, TextAlignment::Center, opt);
                display.put_char_ex(
